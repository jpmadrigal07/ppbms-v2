...
import { I_GlobalState, I_DispatchControlData, I_DispatchControlProofProps } from "../../interfaces";
...
const DispatchControlProof = (props: I_DispatchControlProofProps) => {
    const { 
    ...
    useEffect(() => {
        if(!_.isNil(gAuthData) && !_.isNil(gAuthData!.role)) {
          ...
          const getURLMessengerId = href.replace(
            `${origin}/dispatchcontrolproof?messengerid=`,
            ""
          );
    ...
    const tableData = () => {
        return remappedDispatchControlData?.map((res: I_DispatchControlData) => {
            return (
                <tr>
                    <td style={{textAlign: 'center'}}>{res.recordQty > 0 ? res.recordQty : null}</td>
                    <td style={{textAlign: 'center'}}>{res.dataCycleCode}/{res.pickupDate ? moment(res.pickupDate).format("MMM D, YYYY") : null}</td>
    ...
    return (
        <div className="container dispatch-control-proof">
        ...
        <table className="table table-bordered">
           <thead className="table-header">
               <tr>
                    <th rowSpan={4} style={{paddingBottom: '10px', textAlign: 'center'}}>QUANTITY</th>
                    <th rowSpan={4} style={{paddingBottom: '10px', textAlign: 'center'}}>CYCLECODE/PICK-UP</th>
                    <th rowSpan={4} style={{paddingBottom: '10px', textAlign: 'center'}}>PRODUCT DESCRIPTION</th>
                    <th colSpan={5} style={{textAlign: 'center'}}>DATE RETURNED</th>
               </tr>
               <tr>
                    <th><span style={{visibility: 'hidden'}}>00000</span></th>
                    <th><span style={{visibility: 'hidden'}}>00000</span></th>
                    <th><span style={{visibility: 'hidden'}}>00000</span></th>
                    <th><span style={{visibility: 'hidden'}}>00000</span></th>
                    <th><span style={{visibility: 'hidden'}}>00000</span></th>
